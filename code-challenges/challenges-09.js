...
const roundDecimals = (arr) => {
    // write your code here
    let roundArray = []
    arr.forEach((decimalNum)=> roundArray.push(Math.round(decimalNum)))
    return roundArray
}
// -------------------------------------------------------------------------------------------------------
...
const employeesBonus = (arr) => {
    // write your code here
    arr.forEach((employee,index,array)=>{
        if(employee.workHours > 8){
          let salaryStr = employee.salary
          let salaryInt = parseInt(salaryStr.slice(0,salaryStr.length - 1))
          employee.salary = (salaryInt + 100) + '$' 
         
        }else{
          let salaryStr = employee.salary
          let salaryInt = parseInt(salaryStr.slice(0,salaryStr.length - 1))
          employee.salary = (salaryInt + 50) + '$'
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    // write your code here
    let comoboArray = []
  mouseArray.forEach((mouse,index,array) => {
    keyBoardArray.forEach((keyborde,index,array)=>{
      if(mouse+keyborde <= budget){
       comoboArray.push(mouse+keyborde)
      }
    })
  })

  return comoboArray.sort(function(a,b){
    return b-a
  })[0]
