...
const customerAndAge = (obj) => {
  ...
  let myArrays = Object.entries(obj)
  for(let array of myArrays){
    arrayOutput.push(`Customer Name :${array[0]} , Age :${array[1]}`)
...
const getEntries = (obj) => {
  ...
  let myArrays = Object.entries(obj)
  for(let array of myArrays){
    ingredientsArray.push(`${array[0]}: ${array[1]}`)
  }
  ...
  for (let name of arr){
    for(let obj of courses){
      if(obj.Students.includes(name)){
        outputArray.push(`{name: ${name} , course : ${obj.course}}`)
      }
    }
  }
  return(outputArray)
