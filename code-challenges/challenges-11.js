...
function fullName(arr) {
    // write your code here
    const namesArr = arr.map(currentValue => {
...
function gradesAvg(arr) {
    // write your code here
    const newObjectWithAvg = arr.map(currentValue => {
        let accumulator = 0
        for(let grade of currentValue.gradsList){
            accumulator += grade
        }
        const avg = accumulator/currentValue.gradsList.length
        return {	
            firstName: currentValue.firstName,
            lastName: currentValue.lastName,
            gradsList: currentValue.gradsList,
            avg: avg
        }
    })
    return newObjectWithAvg
}
...
function studentsResult(arr) {
    // write your code here
    const passOrFailed = arr.map(currentValue => {
        if(currentValue.avg >= 50){
            return {
                    firstName: currentValue.firstName,
                    lastName: currentValue.lastName,
                	gradsList: currentValue.gradsList,
        ...
        } else if(currentValue.avg < 50){
            return {firstName: currentValue.firstName,
                lastName: currentValue.lastName,
                gradsList: currentValue.gradsList,
                avg: currentValue.avg,
                result: 'Failed'}
