...
const objLat = (obj) => {
    ...
    let firstName = obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1,obj.firstName.length )
    let lastName = obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1,obj.lastName.length )
    const text = `my name is ${firstName} ${lastName} I am ${obj.age} YO, and I love ${obj.hobby}.`
    return text
...
const cvFormatter = (arr) => {
    ...
    for(let cv of arr){
        if(cv.yearsOfExperience > 1){
            let applicantName
            cv.firstName && cv.lastName ?  applicantName = cv.firstName +" "+ cv.lastName : applicantName = cv.firstName ;
        
      interstedCv.push({fullName:applicantName,tech:cv.tech})
    }
  }
  return interstedCv
};
...
const applicationsStatics = (arr) => {
    ...
    for (let cv of arr){
      ...
      if ((cv.firstName === null && cv.lastName === null) && cv.yearsOfExperience <= 1) {
...
const classesAvg = (data) => {
  ...
  for(let grade of data.grades){
    for(let Class of grade.classes){
      // console.log(Class.classScores)
      
      Class.avg = Math.floor(calculateTotal(Class.classScores)/Class.classScores.length)
      // console.log(Class.avg)
