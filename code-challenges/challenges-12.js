...
const oddFiltration = (arr) => {
    ...
    const oddNumber = arr.filter(num => {
        return num%2 !== 0
    })
    return oddNumber
}
...
const cvsFiltration = (arr) => {
    // write your code here
    const candidatesList = arr.filter(cv => {
        return cv.yearsOfExperience > 4 && cv.firstName !== null && cv.LastName !== null
    })
    let namesArr = []
    for(let candidate of candidatesList){
      const fullName = candidate.firstName+' '+candidate.LastName
      const tech = candidate.tech
      namesArr.push( {
        fullName: fullName,
        tech: tech
    })
...
const vowelsFiltration = (arr) => {
    ...
    const filteredArray = arr.filter(word => {
